...
deepstate_make_angora <- function(){
  ...
  system(paste0("cd ",build_angora,"; sudo cp ./libdeepstate_taint.a /usr/local/lib/"))

  ##building angora fast
  build_angora_fast <- file.path(master,"build_angora_fast")
  Sys.setenv(USE_FAST="1")
  system(paste0("cd ", build_angora_fast," ; ","CXX=",ANGORA_HOME,"/bin/angora-clang++ ","CC=",ANGORA_HOME,
                "/bin/angora-clang cmake -DDEEPSTATE_ANGORA=ON ../"," ; ", "make -j4 -i "))
  system(paste0("cd ",build_angora_fast,"; sudo cp ./libdeepstate_fast.a /usr/local/lib/"))
  }
